...
## A Solução
O Self-Log v1 unifica o registro histórico ao Git. Ao capturar uma entrada, a ferramenta identifica automaticamente o **commit hash** e a **branch** atual, persistindo essas informações no `SELF-LOG.md`. Através da flag `--commit`, o Self-Log permite que a documentação ocorra simultaneamente ao ato de codar, garantindo que o "porquê" esteja sempre a um commit de distância. Por padrão, o commit automático é desativado para garantir controle total do desenvolvedor.
...
Ela automatiza o fluxo: registra sua nota no log, executa `git add .` e cria um commit cujo comentário referencia o hash do log, criando um vínculo bidirecional manual.
...
## Experiência do Usuário
O fluxo é intuitivo. Ao rodar `self-log`, o desenvolvedor registra sua nota e a ferramenta anexa metadados do Git automaticamente. Usuários podem usar `self-log --commit` para registrar o log e realizar o commit do código em um único passo atômico.