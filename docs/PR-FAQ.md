...
## A Solução
O Logloop v1 unifica o registro histórico ao Git. Ao capturar uma entrada, a ferramenta identifica automaticamente o **commit hash** e a **branch** atual, persistindo essas informações no `logloop.md`. Por padrão, o Logloop não realiza commits automaticamente, garantindo controle total do desenvolvedor. Através da flag `--commit` ou configuração via `.loglooprc`, é possível registrar o log e realizar o commit simultaneamente, garantindo que o "porquê" esteja sempre a um commit de distância.
